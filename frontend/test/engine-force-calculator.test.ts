...
const calculator = new EngineForceCalculator()
...
test('wheels in ground moving right', () => {
  // @ts-ignore
  const vehicleModel = JSON.parse(JSON.stringify(vehicle)) as VehicleModel
  // @ts-ignore
  const roomModel = room as RoomModel
  ...
  const expectedMagnitude = vehicleModel.specs.acceleration * calculations.getMass() / 2
  const factMagnitude = sumForces.length ? VectorUtils.getMagnitude(sumForces[0]!) : 0.0
  ...
  expect(factMagnitude).toBeCloseTo(expectedMagnitude, SMALL_DELTA)
  expect(calculations.getGroundContacts().size).toBe(2)
})

test('wheels in ground moving left', () => {
  // @ts-ignore
  const vehicleModel = JSON.parse(JSON.stringify(vehicle)) as VehicleModel
  // @ts-ignore
  const roomModel = room as RoomModel
...
test('full over ground moving right', () => {
  // @ts-ignore
  const vehicleModel = JSON.parse(JSON.stringify(vehicle)) as VehicleModel
  // @ts-ignore
  const roomModel = room as RoomModel
...
test('one wheel over ground moving right', () => {
  // @ts-ignore
  const vehicleModel = JSON.parse(JSON.stringify(vehicle)) as VehicleModel
  // @ts-ignore
  const roomModel = room as RoomModel
